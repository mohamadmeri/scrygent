"""Destructive test suite for the 2-Pass Compiler Planner Node.
...
are handled gracefully, and that the internal self-healing loop
...
def valid_draft_plan() -> DraftPlan:
    """Provide a valid DraftPlan for Pass 1."""
    return DraftPlan(
...
def valid_final_plan() -> Plan:
    """Provide a valid strict Plan for Pass 2."""
    return Plan(
        steps=[
            Step(
                step_id="step_1",
...
class TestRunPlannerNodeExecution:
    ...
    def test_use_case_compiles_two_passes_and_returns_running_status(
        self,
        ...
        """Inject a valid AgentState and mock both LLM passes to succeed.

        Asserts the node returns the exact final Plan object and sets the
        `execution_status` to `"running"`.
        """
        with resilient_call_mock([valid_draft_plan, valid_final_plan]):
...
class TestRunPlannerNodeFailures:
    ...
    def test_aborts_on_service_exhausted_error(
        ...
        assert result["execution_status"] == "aborted"
        assert any("Compiler Pipeline failed: Planner (Parser Pass) did not recover after 3 attempt(s)" in msg for msg in result["error_log"])
...
class TestPlannerSelfHealingLoop:
    """Tests validating the internal Pass 2 correction loop."""

    def test_self_heals_pass_2_validation_error_on_first_retry(
        self,
        planner_state: AgentState,
        valid_draft_plan: DraftPlan,
        valid_final_plan: Plan,
        ...
        """Inject a `ValidationError` on the first Pass 2 attempt, then success.
        ...
        # Pass 1 (success), Pass 2 Attempt 1 (fail), Pass 2 Attempt 2 (success)
        with resilient_call_mock([valid_draft_plan, val_error, valid_final_plan]) as script:
            result = run_planner_node(planner_state)
    ...
    def test_aborts_after_max_self_healing_retries(
        ...
        """Inject a `ValidationError` on all Pass 2 attempts.
        ...
        # Pass 1 (success), Pass 2 Attempt 1 (fail), Attempt 2 (fail), Attempt 3 (fail)
        with resilient_call_mock([valid_draft_plan, val_error, val_error, val_error]):
            result = run_planner_node(planner_state)

        assert result["execution_status"] == "aborted"
        assert any("Planner failed to generate a valid Plan" in msg for msg in result["error_log"])
