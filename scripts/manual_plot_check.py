"""Manual visual inspection script for all plot types.

Run from the project root:
    python scripts/manual_plot_check.py

Images are saved in scripts/plot_outputs/.
"""
import shutil
import pandas as pd
...
# Generate all plots, copy temp files to output_dir
plot_specs = [
...
for ptype, cols, csv_path in plot_specs:
    result = generate_plot(str(csv_path), plot_type=ptype, columns=cols, title=f"Test {ptype}")
